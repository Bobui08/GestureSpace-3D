...
const ParticleShaderMaterial = {
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // Add some depth darkening and glow
      finalColor *= smoothstep(-10.0, 10.0, vDepth * 0.5 + 2.0);
      
      // Add strong emissive glow for vibrant colors
      finalColor += vColor * 0.6 * strength;
      
      gl_FragColor = vec4(finalColor, 1.0 * strength);
    }
  `
};
...
interface FlagSystemProps {
  ...
  pointSize?: number;
  enableWave?: boolean;
  gestureDistance?: number; // 0 to 1 from hand tracking
  hasHands?: boolean;
}

export const FlagSystem: React.FC<FlagSystemProps> = ({
  particleCount = 20000,
  waveStrength = 1.5,
  waveSpeed = 0.8,
  pointSize = 0.35,
  ...
  useFrame((state) => {
    ...
    if (hasHands) {
      // Map 0..1 distance to 0.4..2.0 expansion
      targetExpansion = THREE.MathUtils.mapLinear(gestureDistance, 0, 1, 0.4, 2.0);

      // Boost wave strength if expanding
      const targetWaveBoost = THREE.MathUtils.mapLinear(gestureDistance, 0, 1, 0.5, 2.0);
